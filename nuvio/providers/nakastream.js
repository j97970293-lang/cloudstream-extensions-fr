// =============================================================
  // Provider Nuvio : Nakastream (VF / VOSTFR / MULTI)
  ...
  function detectEndpoint() {
    ...
    return fetch(DOMAINS_URL)
      ...
      .then(function(d) {
        var tld = d.nakastream || FALLBACK_TLD;
        _cachedEndpoint = { base: 'https://nakastream.' + tld, api: 'https://api.nakastream.' + tld + '/api', ref: 'https://nakastream.' + tld + '/' };
        return _cachedEndpoint;
      })
      .catch(function() {
        _cachedEndpoint = { base: 'https://nakastream.' + FALLBACK_TLD, api: 'https://api.nakastream.' + FALLBACK_TLD + '/api', ref: 'https://nakastream.' + FALLBACK_TLD + '/' };
        return _cachedEndpoint;
      });
  }
  ...
  function getTmdbMeta(tmdbId, type) {
    ...
    return fetch(url).then(function(r) { return r.json(); }).then(function(d) {
      ...
      return { name: d.title || d.name || 'Nakastream', year: (d.release_date || d.first_air_date || '').split('-')[0], duration: duration };
    }).catch(function() { return { name: 'Nakastream', year: '', duration: '' }; });
  }
  ...
  function getStreams(tmdbId, mediaType, season, episode) {
    return Promise.all([
      ...
      console.error('[Nakastream]', e.message || e);
      return [];
    });
  }
